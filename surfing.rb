...
class Surfing
  ...
  def call(env)
    ...
    if request.get? && request.path == '/'
      ...
      journal_entry = @db.execute("SELECT * FROM journal_entries WHERE id = #{request.params['id']}").first
      @data = {'journal_entry' => journal_entry}
      render :show
    elsif request.post? && request.path == '/create_journal_entry'
      title, content = request.params["title"], request.params["content"]
      @db.execute("INSERT INTO journal_entries (title, content) VALUES ('#{title}', '#{content}')")
      redirect_to "/"
    elsif request.get? && request.path == '/edit_journal_entry'
      journal_entry = @db.execute("SELECT * FROM journal_entries WHERE id = #{request.params['id']}")
      @data = {'journal_entry' => journal_entry}
      render :edit
      ...
      @db.execute("DELETE FROM journal_entries WHERE id = #{request.params['id']}")
      redirect_to "/"
    elsif request.post? && request.path == '/create_user'
      @db.execute("INSERT INTO users (name, username, password) VALUES ('#{request.params['name']}', '#{request.params['username']}', '#{request.params['password']}')")
      redirect_to "/"
    elsif request.post? && request.path == '/sign_in'
      username, password = request.params['username'], request.params['password']
      result = @db.execute("SELECT * FROM users WHERE username == '#{username}' and password == '#{password}'").first
      if result.any?
